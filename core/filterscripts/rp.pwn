...
CMD:sudungcuuthuong(playerid, params[])
{
	if(bInfo[playerid][pFirstaid] > 0)
	{
		...
		{
			SendClientMessage(playerid, COLOR_GREY, "You're already using first aid!");
		}
	}
	else
	{
...
CMD:muatuicuuthuong(playerid, params[])
{
	if(IsPlayerInRangeOfPoint(playerid, 3.0, 1171.75903, -1319.67737, 14.52226))
	{
		...
		{
			bInfo[playerid][pFirstaid]++;
			SendClientMessage(playerid,COLOR_GREY,"You bought a first aid kit for $300!");
	...
	{
	    SendClientMessage(playerid,COLOR_GREY,"You are not near All Saints Hospital to buy a medkit");
	}
	return 1;
}
